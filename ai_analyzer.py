...
# ============================================================
# 团队垂类分类表（用户提供，共15个）+ 每类一句含义说明（给AI看，定类更准）
# ============================================================

CAT_GUIDE = {
    "남성 의류": "男装、男士穿搭、男士鞋包配饰搭配（标题/标签含남자·남성·맨즈等信号）",
    "여성 의류": "女装、女士穿搭（标题/标签含여자·여성·걸等信号）",
    "완구 및 취미": "玩具、手办、手工DIY、爱好收藏",
    "의복 & 부속품": "不分性别的服饰配件：腰带、帽子、墨镜、包、首饰等",
    "네일": "美甲、指甲护理",
    "생활용품": "生活日用、收纳、清洁、厨房小物",
    "홈 & 가든": "家居装饰、室内布置、园艺",
    "뷰티 & 헬스": "美妆、护肤、美发、身体护理、健康",
    "컴퓨터 및 오피스": "数码外设、办公文具、桌面布置",
    "오토바이 장비 및 부품": "摩托车/自行车装备及配件",
    "전화 및 통신 액세서리": "手机壳、充电器、耳机等手机配件",
    "가구": "家具",
    "도구": "工具、五金",
    "애완동물": "宠物用品、养宠日常",
    "음식": "美食、吃播、做饭",
...
AI_CATEGORY_TABLE = list(CAT_GUIDE.keys())
...
def _build_prompt(briefs: list[dict]) -> str:
    cat_guide = "\n".join(f"- {k}：{v}" for k, v in CAT_GUIDE.items())
    payload = json.dumps(briefs, ensure_ascii=False, indent=1)
    return f"""你是资深韩国YouTube电商网红营销选号专家。

我们的客户：AliExpress（速卖通）韩国站。
带货方向：轻小件、高性价比商品为主——家居好物、平价美妆、服饰配件、生活日用、宠物用品、文具、数码小配件等（轻便、小巧、易冲动下单）。
观众人群：以18-34岁韩国女性为主，但不限女性——男性受众频道只要内容适合挂轻小件商品链接，同样有价值。
合作形式：博主在YouTube视频里挂我们的商品标签带货（种草/开箱haul/测评/日常vlog植入等）。

下面给你 {len(briefs)} 个候选YouTube频道的信息（JSON数组，idx是编号）。请逐个判断三件事：

1. category：先看懂频道在讲什么，再对照下面每个类的含义，选一个最贴切的，类名一字不差地写：
{cat_guide}
性别信号优先：先扫标题/标签里的受众性别信号。出现 남자、남성、맨즈、#남자코디、#남성패션、boy、men 等明显男性信号时，禁止选「여성 의류」，应选「남성 의류」；反之出现 여자、여성、걸、#여자코디 等明显女性信号时，禁止选「남성 의류」。

2. relevance：整数0-100，这个频道适不适合挂轻小件商品链接带货——核心看内容能不能自然植入轻便小巧的平价商品，观众性别只是参考、不是门槛：
- 80-100：内容本身就是种草/消费/好物分享类（开箱haul、测评、日常植入、收纳、美妆、穿搭、宠物、文具、数码小物等），挂轻小件链接非常自然
- 60-79：内容是生活方式/经验分享类（露营、骑行、独居、带娃、办公桌面等），轻小件植入不突兀
- 40-59：沾边但植入机会少，或内容偏大件/重物/高价（整车、大家电、房产、奢侈品）
- 0-39：明显不相关（游戏、新闻、政治、体育赛事解说、纯搞笑、音乐翻唱、猎奇等）
- 注意：男性受众频道（男士穿搭、数码、工具、骑行、宠物等）只要内容适合轻小件种草，照样打高分；不要因为受众不是女性就压分。
